...
enum
{
  ...
  CSMT_DIRECT_CMD,         /* send a command directly to the modem */
  CSMT_SIM_SELECT,         /* CS_SimSlot_t */
  CSMT_SIM_GENERIC_ACCESS, /* csint_sim_generic_access_t */
...
typedef struct
{
  ...
  uint16_t trp_conn_setup_timeout; /* 10 to 1200 hundreds of ms, 0 means infinite,
                                    * default = DEFAULT_TRP_CONN_SETUP_TIMEOUT */
  uint16_t trp_transfer_timeout; /* 0 to 255 ms, 0 means infinite, default = DEFAULT_TRP_TRANSFER_TIMEOUT */
...
typedef struct
{
  ...
  /* following parameters are used only in sendto or receivefrom */
  CS_IPaddrType_t  ip_addr_type;
  CS_CHAR_t        ip_addr_value[MAX_IP_ADDR_SIZE]; /* remote IP address */
  uint16_t         remote_port;
...
typedef struct
{
  CS_sim_generic_access_t *data;
  uint32_t                bytes_received; /* total length of the response received */
  /* only the first data->buffer_rsp_size will be copied
     must know if the buffer response is complete or not */
} csint_sim_generic_access_t;
