...
typedef struct
{
  ...
  uint8_t                flowCtrl_RTS;       /* is RTS flow control activated ? (from AT+IFC) */
  uint8_t                flowCtrl_CTS;       /* is CTS flow control activated ? (from AT+IFC) */
...
typedef struct
{
  ...
  csint_sim_generic_access_t  sim_generic_access;     /* SID_CS_SIM_GENERIC_ACCESS */
