...
/* Size of ICCID head representing the Country Code and the Issuer Identifier */
/* 4 digits and the leading zero */
#define ICCID_HEAD_SIZE        5U

/* Size of MCC + MNC part of the IMSI */
/* 5 digits and the leading zero */
/* Only MNC length of 2 digit is considered. 3 digit MNC are not managed */
#define MCCMNC_HEAD_SIZE       6U

/* Array structure for MMN/MNC - APN association */
typedef struct
{
  uint8_t mccmnc[MCCMNC_HEAD_SIZE];        /* MCC/MNC to which associate APN + username + password
                                              Only MNC length of 2 digit is considered. 3 digit MNC are not managed */
  uint8_t apn[CA_APN_SIZE_MAX];            /* APN to associate to MCC/MNC */
  uint8_t username[CA_USERNAME_SIZE_MAX];  /* Username for APN to associate to MCC/MNC */
  uint8_t password[CA_PASSWORD_SIZE_MAX];  /* Password for APN to associate to MCC/MNC */
} mccmnc_apn_t;

/* Array structure for ICCID head - APN association */
typedef struct
{
  uint8_t iccidhead[ICCID_HEAD_SIZE];      /* head of ICCID to which associate APN + username + password */
  uint8_t apn[CA_APN_SIZE_MAX];            /* APN to associate to CC/II of ICCID */
  uint8_t username[CA_USERNAME_SIZE_MAX];  /* Username for APN to associate to CC/II */
  uint8_t password[CA_PASSWORD_SIZE_MAX];  /* Password for APN to associate to CC/II */
} iccidhead_apn_t;

/* Private variables ---------------------------------------------------------*/

CS_LowPower_status_t cst_lp_status;
...
static CS_SimSlot_t  cst_convert_sim_socket_type(ca_sim_slot_type_t sim_slot_value)
{
  ...
  switch (sim_slot_value)
  {
    ...
    case CA_SIM_INTERNAL_MODEM_SLOT:
      ...
      break;
    default:
      /* If no valid value is recognised, use socket slot */
      enum_value = CS_MODEM_SIM_SOCKET_0;
      break;
  }
  return enum_value;
...
static uint32_t cst_modulo64(uint32_t div, uint32_t val_m, uint32_t val_l)
{
  ...
  /* We want the rest of the division of val_m:val_l by div */
  /* principle : we will subtract different multiples of the divisor which keeps the modulo (but we must manage the */
  /*   deduction in the subtraction) */
  /* We will start by (div * 0x1_0000_0000) that is stored in div_m:div_l) */
  /* then we will divide by 2 the multiple of the divisor (shift to the right by managing the retained) */
  if (div_m != 0U)
  {
    /* Divisor is not null */
    /* we start the calculation by modulating the upper part of the dividend, which means that the multiple of the */
    /*   chosen divisor is necessarily greater than the dividend. */
    /* Example : in 7 = 127 % 10, 10 is necessarily greater than 7 */
    tmp_m = val_m % div_m;

    tmp_l = val_l;

    /* as soon as the upper part is zero, wa can make a simple modulo of the lower part */
    while (tmp_m > 0U)
    {
      if ((div_m > tmp_m) || ((div_m == tmp_m) && (div_l > tmp_l)))
      {
        /* Nothing to do */
        /* multiple of the divisor greater than the dividend, the multiple of the divisor must be reduced. */
        /* see below */
      }
      else if (div_l > tmp_l)
      {
        /* subtraction of the multiple of the divisor with carry. */
        tmp_l = tmp_l - div_l;
        tmp_m--;
        tmp_m = tmp_m - div_m;
      }
      ...
      {
        /* subtraction of the multiple of the divisor without carry. */
        tmp_m = tmp_m - div_m;
        tmp_l = tmp_l - div_l;
      }
      /* Here the multiple of the divisor is reduced */
      /* division by 2 (shift) of the multiple of the divisor. */
      div_l = div_l >> 1;
      if ((div_m & 1U) == 1U)
      {
    ...
    /* The high part is now zero */
    /* We make a simple modulo of the lower part */
    tmp_l = tmp_l % div;
  }
  return tmp_l;
}
...
static void CST_network_reg_callback(void)
{
  PRINT_CELLULAR_SERVICE("CST: CST_network_reg_callback\n\r")
...
void CST_cellular_power_status_callback(CS_LowPower_status_t lp_status)
{
  ...
  PRINT_CELLULAR_SERVICE("CST: CST_cellular_power_status_callback\n\r")
  cst_lp_status.nwk_periodic_TAU = lp_status.nwk_periodic_TAU;
  cst_lp_status.nwk_active_time = lp_status.nwk_active_time;

  CST_send_message(CST_MESSAGE_CS_EVENT, CST_POWER_STATUS_CALLBACK_EVENT);
}
#endif /* (USE_LOW_POWER == 1) */
...
static void CST_location_info_callback(void)
{
  ...
  PRINT_CELLULAR_SERVICE("CST: CST_location_info_callback\n\r")
}
...
static void CST_modem_event_callback(CS_ModemEvent_t event)
{
  PRINT_CELLULAR_SERVICE("CST: CST_modem_event_callback\n\r")
  ...
  if (((uint16_t)event & (uint16_t)CS_MDMEVENT_POWER_DOWN) != 0U)
  {
    ...
    PRINT_CELLULAR_SERVICE("CST: Modem event received: CS_MDMEVENT_POWER_DOWN\n\r")
  ...
  if (((uint16_t)event & (uint16_t)CS_MDMEVENT_LP_ENTER) != 0U)
  {
    ...
    PRINT_CELLULAR_SERVICE("CST: Modem event received: CS_MDMEVENT_LP_ENTER\n\r")
  ...
  {
    /* Modem requests to leave Low Power  */
    PRINT_CELLULAR_SERVICE("CST: Modem event received: CS_MDMEVENT_WAKEUP_REQ\n\r")
    CST_send_message(CST_MESSAGE_CS_EVENT, CST_POWER_MODEM_WAKEUP_EVENT);
  }

  if (((uint16_t)event & (uint16_t)CS_MDMEVENT_LP_LEAVE) != 0U)
  {
    /* Modem Leave Low Power state  */
    PRINT_CELLULAR_SERVICE("CST: Modem event received: CS_MDMEVENT_LP_LEAVE\n\r")
    /* Nothing to do */
  }
#endif /* (USE_LOW_POWER == 1) */
}
...
ca_access_techno_t cst_convert_access_techno(CS_AccessTechno_t access_techno)
{
  ...
  switch (access_techno)
  {
    ...
    case CS_ACT_E_UTRAN_NBS1:
      ...
      break;
    default:
      /* if no valid recognized value, then fall back tu GSM */
      ret = CA_ACT_GSM;
      PRINT_CELLULAR_SERVICE_ERR("CST: Invalid access techno value, use GSM\n\r")
      break;
  }
...
void  CST_modem_sim_init(void)
{
  ...
  if (osCS_sim_select(cst_convert_sim_socket_type(cst_sim_info.active_slot)) != CELLULAR_OK)
  {
    ...
    /* --> force modem reboot */
    PRINT_CELLULAR_SERVICE("CST: CST_modem_sim_init : sim slot requested is not actually in use by the modem.\
  ...
  else
  {
    ...
    else if (cst_cellular_params.target_state == DC_TARGET_STATE_FULL)
    {
      ...
      */
      ...
      PRINT_CELLULAR_SERVICE("CST: --> check SIM First \n\r")
      ...
      {
        /* If apnSendToModem not initialized, initialize it to APN_SEND_TO_MODEM */
        cst_cellular_params.sim_slot[cst_context.sim_slot_index].apnSendToModem =
          (cellular_apn_send_to_modem_t)CA_APN_SEND_TO_MODEM;
      ...
      if (cs_status == CELLULAR_OK)
      {
        ...
        PRINT_CELLULAR_SERVICE("CST: osCDS_get_device_info(IMSI)\n\r")
        ...
        uint8_t mcc_mnc[MCCMNC_HEAD_SIZE];
        uint8_t iccid_head[ICCID_HEAD_SIZE];

        (void)ATutil_convertHexaStringToInt64(cst_imsi_info.u.imsi, 15U, &cst_imsi_high, &cst_imsi_low);
        /* The 5 firsts digit of IMSI are MCC + MNC */
        /* Only MNC length of 2 digit is considered. 3 digit MNC are not managed */
        /* copy the five first digit of the IMSI to the mcc_mnc variable */
        (void)memcpy(mcc_mnc, cst_imsi_info.u.imsi, MCCMNC_HEAD_SIZE);
        mcc_mnc[MCCMNC_HEAD_SIZE - 1U] = 0;

        /* read ICCID */
        cst_imsi_info.field_requested = CS_DIF_ICCID_PRESENT;
        PRINT_CELLULAR_SERVICE("CST: osCDS_get_device_info(ICCID)\n\r")
        (void)osCDS_get_device_info(&cst_imsi_info);
        /* 4 digits (from 3rd to 6th) of ICCID are SIM provider dedicated (representing the Country Code and */
        /* the Issuer Identifier) */
        /* copy these four digit of the ICCID to the iccid_head variable */
        (void)memcpy(iccid_head, &(cst_imsi_info.u.iccid[2]), ICCID_HEAD_SIZE);
        /* Add final \0 to the iccid_head string */
        iccid_head[ICCID_HEAD_SIZE - 1U] = (uint8_t)'\0';

        PRINT_CELLULAR_SERVICE("CST: --> IMSI: %lx%lx\n\r", cst_imsi_high, cst_imsi_low)
        PRINT_CELLULAR_SERVICE("CST: --> MCC/MNC: %s (2 digit MNC only)\n\r", mcc_mnc)
        PRINT_CELLULAR_SERVICE("CST: --> ICCID: %s\n\r", cst_imsi_info.u.iccid)
        PRINT_CELLULAR_SERVICE("CST: --> ICCID CC/II: %s\n\r", iccid_head)
        ...
        {
          bool    is_apn_found;
          uint8_t apn_found[CA_APN_SIZE_MAX];            /* APN to associate to SIM */
          uint8_t username_found[CA_USERNAME_SIZE_MAX];  /* Username for APN to associate to SIM */
          uint8_t password_found[CA_PASSWORD_SIZE_MAX];  /* Password for APN to associate to SIM */

          /* If no APN is in data cache, then check if MCC/MNC is present in the */
          /* MCC/MNC <-> APN table or in the ICCID CC/II <-> APN table            */
          if (cst_cellular_params.sim_slot[cst_context.sim_slot_index].apnPresent == false)
          {
            ...
            */
            is_apn_found = false;
            apn_found[0] = (uint8_t)'\0';
            username_found[0] = (uint8_t)'\0';
            password_found[0] = (uint8_t)'\0';

            /* Parse the MCC/MNC <-> APN table to search for the actual MCC/MNC read in the SIM */
            /* Only MNC length of 2 digit is considered. 3 digit MNC are not managed */
            for (uint8_t i = 0U; i < MCCMNC_APN_MAX; i++)
            {
              if (memcmp(mcc_mnc, mccmnc_apn_a[i].mccmnc, sizeof(mcc_mnc)) == 0)
              {
                /* MCC/MNC found, store the corresponding APN parameters for future use */
                is_apn_found = true;
                (void)memcpy(apn_found,
                             mccmnc_apn_a[i].apn,
                             crs_strlen(mccmnc_apn_a[i].apn) + 1U);
                (void)memcpy(username_found,
                             mccmnc_apn_a[i].username,
                             crs_strlen(mccmnc_apn_a[i].username) + 1U);
                (void)memcpy(password_found,
                             mccmnc_apn_a[i].password,
                             crs_strlen(mccmnc_apn_a[i].password) + 1U);

                PRINT_CELLULAR_SERVICE("CST: --> Found corresponding APN: \"%s\" in MCC/MNC table\n\r",
                                       mccmnc_apn_a[i].apn)
              }
            }
            /* Parse the ICCID head <-> APN table to search for the actual ICCID head read in the SIM */
            for (uint8_t i = 0U; i < ICCIDHEAD_APN_MAX; i++)
            {
              if (memcmp(iccid_head, iccidhead_apn_a[i].iccidhead, sizeof(iccid_head)) == 0)
              {
                /* ICCID head found, store the corresponding APN parameter for future use */
                is_apn_found = true;
                (void)memcpy(apn_found,
                             iccidhead_apn_a[i].apn,
                             crs_strlen(iccidhead_apn_a[i].apn) + 1U);
                (void)memcpy(username_found,
                             iccidhead_apn_a[i].username,
                             crs_strlen(iccidhead_apn_a[i].username) + 1U);
                (void)memcpy(password_found,
                             iccidhead_apn_a[i].password,
                             crs_strlen(iccidhead_apn_a[i].password) + 1U);
                PRINT_CELLULAR_SERVICE("CST: --> Found corresponding APN: \"%s\" in ICCID CC/II table\n\r",
                                       iccidhead_apn_a[i].apn)
              }
            }
            /* if an APN has been found in the MCC/MNC <-> APN table or in the ICCID CC/II <-> APN table, */
            /* then use it with its username and password to configure the modem */
            if (is_apn_found)
            {
              ...
              if (cst_cellular_params.sim_slot[cst_context.sim_slot_index].apnSendToModem ==
                  (cellular_apn_send_to_modem_t)CA_APN_SEND_TO_MODEM)
              {
                PRINT_CELLULAR_SERVICE("CST: --> call CST_modem_define_pdn with APN found in table: \"%s\"\n\r",
                                       apn_found)
                /* Set APN in modem data structure */
                CST_modem_define_pdn(apn_found,
                                     username_found,
                                     password_found);
              }
            }
            else /* an APN has NOT been found in the MCC/MNC <-> APN table nor in the ICCID CC/II <-> APN table */
            {
              if (cst_cellular_params.sim_slot[cst_context.sim_slot_index].apnSendToModem ==
                  (cellular_apn_send_to_modem_t)CA_APN_SEND_TO_MODEM)
              {
                /* APN not found in any table, and, APN to be send to modem */
                PRINT_CELLULAR_SERVICE("CST: --> MCC/MNC or CC/II not in table \
call CST_modem_define_pdn with APN not to be send to modem\n\r")
          ...
          else /* APN is present in structure */
          {
            if (crs_strlen(cst_cellular_params.sim_slot[cst_context.sim_slot_index].apn) == 0U)
            {
              ...
              PRINT_CELLULAR_SERVICE("CST: --> APN is empty string \
call CST_modem_define_pdn with APN not to be send to modem\n\r")
              CST_modem_define_pdn(NULL, (uint8_t *)"", (uint8_t *)"");
            }
            else
            {
              ...
              PRINT_CELLULAR_SERVICE("CST: --> call CST_modem_define_pdn with stored APN: \"%s\"\n\r",
    ...
    else
    {
      ...
      /* subscribe to needed callbacks */
      CST_subscribe_all_net_events();

      /* Update Data Cache with all device information */
      CST_get_device_all_infos(cst_cellular_params.target_state);
      if (cst_cellular_params.target_state != DC_TARGET_STATE_SIM_ONLY)
      {
        /* Target state is not "sim_only", start attachment process */
...
/**
  * @brief  subscribes to all network events
  * @param  -
  * @retval -
  */
static void CST_subscribe_all_net_events(void)
{
  PRINT_CELLULAR_SERVICE("CST: Subscribe URC events: Network registration\n\r")
  ...
  PRINT_CELLULAR_SERVICE("CST: Subscribe URC events: Location info\n\r")
...
static void CST_fill_nfmc_tempo(uint32_t imsi_high, uint32_t imsi_low)
{
  ...
  if (cst_cellular_params.nfmc_active != 0U)
  {
    ...
    for (i = 0U; i < CA_NFMC_VALUES_MAX_NB; i++)
    {
      ...
      PRINT_CELLULAR_SERVICE("CST: VALUE/TEMPO %ld/%ld\n\r",  cst_cellular_params.nfmc_value[i],
                             cst_nfmc_context.tempo[i])
    }
...
static void CST_get_device_all_infos(dc_cs_target_state_t  target_state)
{
  ...
  PRINT_CELLULAR_SERVICE("CST: CST_get_device_all_infos\n\r")
  sim_poll_count = 0U;
  ...
  if (osCDS_get_device_info(&cst_device_info) == CELLULAR_OK)
  {
    ...
    PRINT_CELLULAR_SERVICE("CST: --> IMEI: %s\n\r", cst_device_info.u.imei)
  }
  else
  {
    cst_cellular_info.imei[0] = 0U;
    PRINT_CELLULAR_SERVICE("CST --> IMEI error\n\r")
  }
  ...
  if (osCDS_get_device_info(&cst_device_info) == CELLULAR_OK)
  {
    ...
    PRINT_CELLULAR_SERVICE("CST: --> MANUFACTURER: %s\n\r", cst_device_info.u.manufacturer_name)
  }
  else
  {
    cst_cellular_info.manufacturer_name[0] = 0U;
    PRINT_CELLULAR_SERVICE("CST: --> Manufacturer Name error\n\r")
  }
  ...
  if (osCDS_get_device_info(&cst_device_info) == CELLULAR_OK)
  {
    ...
    PRINT_CELLULAR_SERVICE("CST: --> MODEL: %s\n\r", cst_device_info.u.model)
  }
  else
  {
    cst_cellular_info.model[0] = 0U;
    PRINT_CELLULAR_SERVICE("CST: --> Model error\n\r")
  }
  ...
  if (osCDS_get_device_info(&cst_device_info) == CELLULAR_OK)
  {
    ...
    PRINT_CELLULAR_SERVICE("CST: --> REVISION: %s\n\r", cst_device_info.u.revision)
  }
  else
  {
    cst_cellular_info.revision[0] = 0U;
    PRINT_CELLULAR_SERVICE("CST: --> Revision error\n\r")
  }
  ...
  if (osCDS_get_device_info(&cst_device_info) == CELLULAR_OK)
  {
    ...
    PRINT_CELLULAR_SERVICE("CST: --> SERIAL NBR: %s\n\r", cst_device_info.u.serial_number)
  }
  else
  {
    cst_cellular_info.serial_number[0] = 0U;
    PRINT_CELLULAR_SERVICE("CST: --> Serial Number error\n\r")
  }
  ...
  if (target_state == DC_TARGET_STATE_FULL)
  {
    ...
    while (end_of_loop != false)
    {
      ...
      if (cs_status == CELLULAR_OK)
      {
        ...
        PRINT_CELLULAR_SERVICE("CST: --> IMSI: %s\n\r", cst_device_info.u.imsi)
...
static void CST_modem_define_pdn(uint8_t *apn, uint8_t *username, uint8_t *password)
{
  ...
  PRINT_CELLULAR_SERVICE("CST: osCDS_define_pdn()\n\r")
                     ...
                     uint8_t fail_max)
{
  ...
  PRINT_CELLULAR_SERVICE("CST: CST_config_fail: %s Fail !!!\r\n", msg_fail)
  ...
  *fail_count = *fail_count + 1U;
  ...
  else
  {
    ...
    PRINT_CELLULAR_SERVICE_ERR("CST: CST_set_fail_state %d - count %d/%d FATAL !!!\n\r",
                               fail_cause,
                               cst_context.global_retry_count,
                               *fail_count)
...
CS_Status_t CST_set_signal_quality(void)
{
  ...
  if (osCS_get_signal_quality(&sig_quality) == CELLULAR_OK)
  {
    ...
    PRINT_CELLULAR_SERVICE("CST: Sig quality rssi : %d\n\r", sig_quality.rssi)
    PRINT_CELLULAR_SERVICE("CST: Sig quality ber  : %d\n\r", sig_quality.ber)
  }
  else
  {
    ...
    {
      PRINT_CELLULAR_SERVICE("CST: --> Modem signal quality error max\n\r")
      cst_context.csq_count_fail = 0U;
...
{
  PRINT_CELLULAR_SERVICE("CST: Subscribe modems events\n\r")
  ...
  PRINT_CELLULAR_SERVICE("CST: osCDS_subscribe_modem_event()\n\r")
  ...
  if (cs_status == CELLULAR_OK)
  {
    ...
    else /* device registered to network */
    {
      ...
      if (((uint16_t)reg_status.optional_fields_presence & (uint16_t)CS_RSF_FORMAT_PRESENT) != 0U)
      {
        ...
        (void)dc_com_write(&dc_com_db,  DC_CELLULAR_SIGNAL_INFO, (void *)&signal_info, sizeof(signal_info));
        PRINT_CELLULAR_SERVICE("CST: ->operator_name = %s", reg_status.operator_name)
      }
    }
  }
  else
