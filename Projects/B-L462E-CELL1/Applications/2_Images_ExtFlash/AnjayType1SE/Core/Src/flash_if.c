...
HAL_StatusTypeDef FLASH_EXT_If_Erase_Size(void *pStart, uint32_t uLength)
{
  ...
  avs_log(flash, INFO, "Erase external flash");
  ...
  for (i = 0 ; i < ((uLength - 1U) / ERASE_BLOC_SIZE) + 1U; i++)
  {
    if (BSP_QSPI_EraseBlock(address, BSP_QSPI_ERASE_64K) != BSP_ERROR_NONE)
    {
      avs_log(flash, ERROR, "No such address or sth error %d", i);
      e_ret_status = HAL_ERROR;
    }
...
HAL_StatusTypeDef FLASH_EXT_If_Write(void  *pDestination, const void *pSource, uint32_t uLength)
{
  ...
  if (BSP_QSPI_Write((uint8_t *) pSource, (uint32_t) pDestination - EXTERNAL_FLASH_ADDRESS, uLength) != BSP_ERROR_NONE)
  {
    return HAL_ERROR;
  }
  else
...
HAL_StatusTypeDef FLASH_EXT_If_Read(void *pDestination, const void *pSource, uint32_t uLength)
{
  ...
  if (BSP_QSPI_Read((uint8_t *) pDestination, (uint32_t) pSource - EXTERNAL_FLASH_ADDRESS, uLength) != BSP_ERROR_NONE)
  {
    return HAL_ERROR;
  }
  else
...
static uint32_t GetPage(uint32_t uAddr)
{
  return (uAddr - FLASH_BASE) / FLASH_PAGE_SIZE;;
}
